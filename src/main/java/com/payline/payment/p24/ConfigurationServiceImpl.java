...
public class ConfigurationServiceImpl implements ConfigurationService {
    ...
    public static void checkHttpConnection(P24CheckConnectionRequest request, Map<String, String> errors) {
        try {
            // create the body
            ...
            if (response.code() != 200) {
                errors.put(ContractParametersCheckRequest.GENERIC_ERROR, "Probleme reseau");
            } else {
    ...
    public static void checkSoapConnection(P24TestAccessRequest request, Map<String, String> errors) {
        ...
        if (soapResponseMessage != null) {
            ...
            errors.put(ContractParametersCheckRequest.GENERIC_ERROR, "probleme de connection soap");
        }
    }
    ...
    public List<AbstractParameter> getParameters(Locale locale) {
        ...
        // time limit for a payment
        final InputParameter timeLimit = new InputParameter();
        timeLimit.setKey(P24Constants.TIME_LIMIT);
        timeLimit.setLabel(localization.getSafeLocalizedString("contract.timeLimit.label", locale));
        timeLimit.setDescription(localization.getSafeLocalizedString("contract.timeLimit.description", locale));
        ...
        final InputParameter shipping = new InputParameter();
        shipping.setKey(P24Constants.SHIPPING);
        shipping.setLabel(localization.getSafeLocalizedString("contract.shipping.label", locale));
        shipping.setDescription(localization.getSafeLocalizedString("contract.shipping.description", locale));
        shipping.setRequired(false);

        parameters.add(shipping);

        // wait for result
        final ListBoxParameter waitForResult = new ListBoxParameter();
        Map<String, String> waitForResultList = new HashMap<>();
        waitForResultList.put("0", "contract.no");
        waitForResultList.put("1", "contract.yes");
        waitForResult.setList(waitForResultList);
        waitForResult.setKey(P24Constants.WAIT_FOR_RESULT);
        waitForResult.setLabel(localization.getSafeLocalizedString("contract.waitForResult.label", locale));
        waitForResult.setDescription(localization.getSafeLocalizedString("contract.waitForResult.description", locale));
        waitForResult.setRequired(false);

        parameters.add(waitForResult);
    ...
    private void addChannelsListBoxes(List<AbstractParameter> parameters, Locale locale) {
        ...
        listBoxList.put("true", "true");
        listBoxList.put("false", "false");
